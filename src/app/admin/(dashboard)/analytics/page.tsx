          ...
          <div className="flex flex-col gap-2">
            {sales.map((sale) => (
              <div key={sale.id} className="flex items-center gap-3 text-sm">
                <span className="w-40 shrink-0 truncate text-neutral-600 dark:text-neutral-400">{sale.assetName}</span>
            ...
            <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
              {sales.map((sale) => (
                <tr key={sale.id}>
                  ...
                  <td className="px-4 py-2 text-neutral-500">{formatDate(sale.soldAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
