...
export default async function TicketDetailPage({
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <Link href="/admin/tickets" className="text-sm text-neutral-500 hover:underline">
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-3 rounded-lg border border-neutral-300 bg-white p-4 text-sm dark:border-neutral-800 dark:bg-neutral-900">
        <div>
          <span className="text-neutral-500">Email:</span> {ticket.email}
        </div>
        <div>
          <span className="text-neutral-500">Submitted:</span> {formatDate(ticket.createdAt)}
        </div>
        <div>
          <span className="text-neutral-500">Category:</span> {CATEGORY_LABELS[ticket.category]}
        </div>
        <div className="col-span-2">
          <span className="text-neutral-500">Description:</span> {ticket.description}
        </div>
